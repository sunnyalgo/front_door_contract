...
contract Recruitment is Ownable, ReentrancyGuard {
  ...
  mapping(uint32 => FrontDoorStructs.Job) public jobList;
  mapping(address => uint32[]) public referralIndex;
  mapping(uint32 => FrontDoorStructs.Referral) public referralList;
  ...
  mapping(uint32 => FrontDoorStructs.Candidate[]) public candidateListForJob; // list of candidates for a job
  mapping (uint32 => FrontDoorStructs.Candidate) public jobCandidatehire;
  address acceptedTokenAddress;
  // Company address  to  candiate address  gives score to company
  mapping(address => mapping(address => uint32)) public companyaddressToScore;
  //  Company address  to candidate address  to score giving By company
  mapping(address => mapping(address => uint32)) public companyAddressToCandidateScore;
  ...
  //  Counters
  uint32 private jobIdCounter = 1;
  uint32 private referralCounter = 1;
